...
- CLI-surface coverage: which adapter verbs (== kicad-cli subcommands, per
  `runner/verbs.py`) the discovered cases actually exercise. Unexercised verbs are the
  gap list.
...
Since [DL-0025] a case names no verb -- the verbs a case exercises are derived the same
way the engine derives them (`runner.engine.battery_for`/`answer_for_extra`/
`LOADER_VERB`), not read off a `[[check]]` list that no longer exists.
"""
...
def build_coverage_report(cases: list[Case]) -> CoverageReport:
    ...
    for case in cases:
        try:
            ...
            kind = _engine.input_kind(input_path)
        except ValueError:
            kind = None
        if kind is not None:
            if case.polarity == "failure":
                report.exercised_verbs.add(_engine.LOADER_VERB[kind])
            else:
                for answer in _engine.battery_for(kind):
                    report.exercised_verbs.add(answer.verb)
                for name in case.extra:
                    report.exercised_verbs.add(_engine.answer_for_extra(name).verb)
